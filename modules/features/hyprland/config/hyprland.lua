...
local function key(suffix) return mod .. " + " .. suffix end
...
hl.monitor({ output = "desc:Samsung Electric Company Odyssey G61SD", mode = "2560x1440@240", position = "0x0", scale = 1 })
hl.monitor({ output = "desc:Samsung Electric Company LS27A600U", mode = "2560x1440@75", position = "2560x0", scale = 1, transform = 1 })
for i = 1, 5 do hl.workspace_rule({ workspace = tostring(i), monitor = "desc:Samsung Electric Company Odyssey G61SD" }) end
for i = 6, 9 do hl.workspace_rule({ workspace = tostring(i), monitor = "desc:Samsung Electric Company LS27A600U" }) end
hl.config({ input = { kb_layout = "us", kb_variant = "intl", follow_mouse = 1,
  touchpad = { natural_scroll = false }, kb_options = "caps:shiftlock",
  repeat_rate = 40, repeat_delay = 600 } })

bind(key("RETURN"), exec(term_here))
bind(key("W"), hl.dsp.window.kill()); bind(key("F"), hl.dsp.window.fullscreen())
bind(key("T"), hl.dsp.window.float({ action = "toggle" })); bind(key("E"), hl.dsp.layout("togglesplit"))
bind(key("P"), hl.dsp.window.pseudo()); bind(key("SHIFT + P"), hl.dsp.window.pin())
bind(key("SHIFT + R"), exec(hyprctl .. " reload")); bind(key("SHIFT + Q"), hl.dsp.exit())
...
bind("PRINT", exec(grimblast .. " copy output")); bind("SHIFT + PRINT", exec(grimblast .. " copy area"))
...
for _, pair in ipairs({ {"H","l"},{"L","r"},{"K","u"},{"J","d"} }) do
  ...
  bind(key("SHIFT + " .. pair[1]), hl.dsp.window.move({ direction = pair[2] }))
end
bind(key("TAB"), hl.dsp.focus({ workspace = "previous" })); bind(key("BRACKETRIGHT"), hl.dsp.focus({ workspace = "e+1" }))
bind(key("BRACKETLEFT"), hl.dsp.focus({ workspace = "e-1" })); bind(key("PERIOD"), exec(hyprctl .. " dispatch focusmonitor +1"))
...
bind(key("SHIFT + I"), exec("uwsm app -- $TERMINAL -e " .. bash .. " -c '" .. fastfetch .. "; read -rp \\\"Press enter to close...\\\"'"))
bind(key("SHIFT + B"), exec("uwsm app -- " .. firefox))
for i = 1, 9 do bind(key(tostring(i)), hl.dsp.focus({ workspace = i })); bind(key("SHIFT + " .. i), hl.dsp.window.move({ workspace = i })) end
...
bind(key("mouse:272"), hl.dsp.window.drag(), { mouse = true }); bind(key("mouse:273"), hl.dsp.window.resize(), { mouse = true })
...
hl.config({ animations = { enabled = true },
  ...
  decoration = { rounding = 20, rounding_power = 2, shadow = { enabled = true, range = 4, render_power = 3 }, blur = { enabled = true, size = 3, passes = 2, vibrancy = 0.1696 } },
...
hl.window_rule({ name = "bitwarden-workspace", match = { class = "^(Bitwarden)$" } })
hl.layer_rule({ name = "noctalia", match = { namespace = "noctalia-background-.*" }, blur = true, ignore_alpha = 0.5 })
hl.layer_rule({ name = "noctalia-shell-region", match = { namespace = "noctalia-shell:regionSelector" }, no_anim = true })
hl.on("hyprland.start", function() hl.exec_cmd("uwsm app -- " .. noctalia) end)
