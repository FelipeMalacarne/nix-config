{ ... }:
{
  ...
  flake.homeModules.hyprland =
    ...
    let
      ...
      shellCommand =
        name:
        if shellCommands.${name} == null || shellCommands.${name} == "" then ":" else shellCommands.${name};
      shellCommandJson = name: builtins.toJSON (shellCommand name);
      ...
      '';
      commandPaths = {
        "@LAUNCHER@" = shellCommandJson "launcher";
        "@DASHBOARD@" = shellCommandJson "dashboard";
        "@SETTINGS@" = shellCommandJson "settings";
        "@SESSION@" = shellCommandJson "session";
        "@LOCK@" = shellCommandJson "lock";
